...
const ExamSearch = dynamic(() => import("@/components/search/ExamSearch"), {
  ssr: false,
});
  ...
  return (
    <>
      <header className="h-16 border-b border-border bg-card/80 backdrop-blur-sm flex items-center px-6 gap-4 sticky top-0 z-40">
        ...
        {/* Search trigger — full button, opens ExamSearch palette */}
        <button
          onClick={() => setSearchOpen(true)}
          className="hidden md:flex items-center gap-2 text-muted-foreground h-9 px-4 rounded-xl border border-border/60 bg-background hover:bg-accent hover:text-foreground transition-colors text-sm min-w-[220px]"
        >
          <Search className="w-4 h-4 shrink-0" />
          <span className="flex-1 text-left">Search exams...</span>
          <kbd className="text-xs bg-muted px-1.5 py-0.5 rounded border border-border text-muted-foreground">
            ⌘K
          </kbd>
        </button>
      ...
      {/* Search palette — rendered outside header to avoid z-index issues */}
      <ExamSearch open={searchOpen} onClose={() => setSearchOpen(false)} />
    </>
  );
}
